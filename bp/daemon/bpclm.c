...
//#include "selectcla.c"
	char protocolName[MAX_CL_PROTOCOL_NAME_LEN];

	switch (classReqd)
	{
	case BP_PROTOCOL_STREAMING:
		istrcpy(protocolName, "bssp", sizeof protocolName);
		break;
	default:
		istrcpy(protocolName, "ltp", sizeof protocolName);
	}
			...
			ductElt = sdr_list_next(sdr, ductElt))
	{
		...
		if (strcmp(protocol->name, protocolName) == 0)
		{
#ifdef MULTIDUCTS_DEBUG
			printf("Outduct %s for %s selected.\n", outduct->name, protocol->name);
#endif
			return 1;	/*	This duct is usable.	*/
		}
	}
