...
////////////////////// HY TEST
                        char * tmp = (char *) malloc (segmentLength + 5);		//65 + 5

			tmp[0] = 0xAA;					//Sync
                        tmp[1] = (segmentLength >> 8) & 0xFF;		// Length
                        tmp[2] = segmentLength;				// Length
                        tmp[3] = 0x86;					// command
                        tmp[4] = 0x00;					// Packet number

                        memcpy(&tmp[5], segment, segmentLength);
                        int totalLength = segmentLength + 5;
//////////////////////

////////////////////// segment -> tmp, segmentLength -> totalLength
			bytesSent = sendSegmentByUDP(rtp.linkSocket, tmp,
					totalLength, peerInetName);
//////////////////////
